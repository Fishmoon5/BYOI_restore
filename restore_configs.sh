...
clear_config_vtysh() {
  ...
  case "$config" in
  *"no ipv6 forwarding"*)
    config="${config#*no ipv6 forwarding}"
    ;;
...
restore_config() {
  local new_config_file="$config_path/$3.txt"
  local router_ip="${2%???}"
  local router_name="$3"
...
if [[ "$#" -ne 4 ]] || [[ ! -e "$1" ]]; then
  ...
  echo "Usage: $0 <saved_config_directory> <router id> <router name> vtysh"
  echo "Example: $0 configs/ 158.15.10.1/16 LOND vtysh"
  exit 1
fi
